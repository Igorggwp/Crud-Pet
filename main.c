...
void logo()
{
    SetConsoleOutputCP(CP_UTF8); // Configura a saida do console para UTF-8 permitindo a exibicao dos caracteres abaixo
...
int init(int *tamanho)
{
    ...
    { // Le o arquivo linha por linha
        ...
        if (cont == 0)
        {
            ...
            if (mal == 0)
            { // Alocação de memoria para a struct dados e inicializacao de campos
...
int finit(int *tamanho)
{
    ...
    if (file == NULL)
    {
        printf("Problemas na gravação do banco de dados.\n");
        return 0;
    }
    for (int i = 0; i < *tamanho; i++)
    { // Itera sobre os registros e os escreve no arquivo
